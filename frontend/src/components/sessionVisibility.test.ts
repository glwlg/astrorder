...
it('swipes through native-open sessions with infinite loop', () => {
 ...
 expect(adjacentOpenSession(rows, rows[2], 1)?.id).toBe('open-a')
 expect(adjacentOpenSession(rows, rows[0], -1)?.id).toBe('open-b')
 expect(adjacentOpenSession(rows, rows[2], 1, false)).toBeUndefined()
 expect(adjacentOpenSession(rows, rows[0], -1, false)).toBeUndefined()
})
