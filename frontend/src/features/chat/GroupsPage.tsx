...
export function GroupsPage() {
    ...
    void loadGroups().then((items) => {
      ...
      if (groupId) {
        ...
        if (found) setActiveGroup(found)
          ...
          void api.getBotGroup(groupId).then((res) => {
  ...
  if (activeGroup) {
    return (
      <div className="route-page chat-page">
        <BotGroupChatPage group={activeGroup} agents={agents} />
      </div>
    )
  }
  ...
  return (
    <Center style={{ flex: 1, minHeight: '100%', padding: 24 }}>
      <Paper withBorder radius="lg" p="xl" style={{ maxWidth: 460, textAlign: 'center' }}>
        <Stack align="center" gap="md">
          <IconUsers size={38} color="var(--astr-indigo, #5b6cff)" stroke={1.5} />
          <Title order={3}>群聊协作 (Bots)</Title>
          <Text size="sm" c="dimmed">
            {loading ? '正在加载群聊会话…' : '当前暂无活跃群聊。支持将多个 Agent 汇聚于同一群聊，通过 @ 协同接力完成复合任务。'}
          </Text>
          <Button
            color="indigo"
