...
import { loadProjectAppearance, purgeProjectPreferences } from '../../components/projectAppearance'
...
export function MobileWorkspace() {
  ...
  return <div className="mobile-workspace" data-mobile-shell="independent" onTouchStartCapture={handleWorkspaceTouchStart} onTouchMoveCapture={handleWorkspaceTouchMove} onTouchEndCapture={handleWorkspaceTouchEnd} onTouchCancelCapture={() => { edgeTouch.current = null }}>
    ...
    <main className="m-main">{selected ? <MobileSessionDeck sessionKey={key} cut={sessionTransition} drag={sessionDrag}>
      ...
      <MobileTranscript messages={messages} busy={busy} loadOlder={() => resources.messages.fetchNextPage()} hasOlder={!!resources.messages.hasNextPage} loadingOlder={resources.messages.isFetchingNextPage} onMessageAction={anchor => setMessageAction({ ...anchor, sessionKey: key })} onImage={setImage} onSwipe={switchSession} onSwipePreview={setSessionDrag} />
      ...
      <div className="m-composer"><input hidden ref={fileInput} type="file" multiple accept="image/*,audio/*,.pdf,.txt,.md,.json,.csv,.log" onChange={e => { updateFiles([...files, ...Array.from(e.target.files || [])]); e.target.value = '' }} />
        ...
        <Menu position="top-end" width={230} withinPortal>
          ...
          <Menu.Dropdown>
            ...
            <Menu.Divider />
            <Menu.Item color="red" leftSection={<IconPlayerStop size={17} />} onClick={() => void stopAll()}>终止全部任务</Menu.Item>
          </Menu.Dropdown>
        </Menu>
    ...
    {sheet && <div className={`m-backdrop ${sheet === 'sessions' ? 'm-session-backdrop' : ''}`} onClick={() => setSheet(null)} onTouchStartCapture={handleDrawerTouchStart} onTouchMoveCapture={handleDrawerTouchMove} onTouchEndCapture={handleDrawerTouchEnd} onTouchCancelCapture={handleDrawerTouchCancel}><section ref={node => { drawerSheet.current = node }} style={sheet === 'sessions' && drawerOffset !== null ? { transform: `translate3d(${drawerOffset}px, 0, 0)`, transition: drawerSettling ? 'transform .22s cubic-bezier(.2,.8,.2,1)' : 'none', animation: 'none' } : undefined} className={`m-sheet ${sheet === 'sessions' ? 'm-session-sheet' : ''}`} role="dialog" aria-label={sheet === 'sessions' ? '会话列表' : '详情'} onClick={e => e.stopPropagation()} onTouchStart={handleDrawerTouchStart} onTouchMove={handleDrawerTouchMove} onTouchEnd={handleDrawerTouchEnd} onTouchCancel={handleDrawerTouchCancel}><div className="m-handle" /><header><h2>{({ sessions: '会话', status: '运行状态', task: '任务详情', models: '选择模型', queue: '消息队列', connections: '连接管理' })[sheet]}</h2><button aria-label="关闭面板" onClick={() => setSheet(null)}><IconX size={20} /></button></header>
      ...
      {sheet === 'sessions' && <div style={{ display: 'flex', gap: 8, padding: '8px 14px' }}><AgentSessionFilter agents={agents} value={agentFilter} onChange={updateAgentFilter} /><button aria-label="新建会话" onClick={() => { setCreateProject(null); setCreateOpened(true) }}><IconPlus size={20} /></button></div>}
      {sheet === 'sessions' && <><input className="m-search" aria-label="搜索会话" placeholder="搜索会话" value={search} onChange={e => setSearch(e.target.value)} /><nav className="m-filters">{[['all','全部'],['unread','未读'],['open','开放中'],['pinned','置顶'],['recent','24小时']].map(([id,label]) => <button className={filter === id ? 'active' : ''} key={id} onClick={() => setFilter(id)}>{label}</button>)}</nav><MobileSessionDrawer groups={groups} pins={pins} selectedKey={key} appearance={appearance} onSelect={select} onCreate={project => { setCreateProject(project); setCreateOpened(true) }} onDeleteProject={handleDeleteProject} onDeleteSession={handleDeleteSession} onPin={s => { const next = { ...pins, [scopeKey(s.agent_id,s.id)]: !pins[scopeKey(s.agent_id,s.id)] }; setPins(next); localStorage.setItem('astrorder_pinned_sessions', JSON.stringify(next)) }} /></>}
