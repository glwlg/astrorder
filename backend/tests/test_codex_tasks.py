...
def test_native_runtime_events_update_durable_tasks(tmp_path):
    ...
    try:
        ...
        notify('item/started', item=command)
        task = tasks()['codex:background:cmd']
        assert task['status'] == 'running'
        assert task['target_id'] is None  # No unsupported per-process stop action.
        created_at = task['created_at']
        notify('item/commandExecution/outputDelta', itemId='cmd', delta='passed')
        assert tasks()['codex:background:cmd']['logs'][0]['text'] == 'passed'
        notify('item/completed', item={**command, 'status': 'completed', 'exitCode': 1, 'aggregatedOutput': 'failed'})
        assert tasks()['codex:background:cmd']['status'] == 'failed'
        assert tasks()['codex:background:cmd']['created_at'] == created_at
...
def test_disconnect_and_reconnect_reconcile_tasks(tmp_path):
    ...
    try:
        ...
        assert store.list_tasks(connection.agent_id, SID)[0]['status'] == 'running'
        connection._closed()
        assert store.list_tasks(connection.agent_id, SID)[0]['status'] == 'unknown'
        connection.connect()
        # A stale start in history does not establish that the process survived.
        assert store.list_tasks(connection.agent_id, SID)[0]['status'] == 'unknown'
        connection._closed()
        command['status'] = 'completed'
        command['aggregatedOutput'] = 'done offline'
        connection.connect()
        assert store.list_tasks(connection.agent_id, SID)[0]['status'] == 'completed'
        assert store.list_tasks(connection.agent_id, SID)[0]['logs'][0]['text'] == 'done offline'
